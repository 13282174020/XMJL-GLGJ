...
# 新的函数实现
NEW_FUNCTION = '''def get_chapter_content_template(section_title, requirement_content=''):
    """根据章节标题返回预定义的内容模板
    优化：区分信息型字段和描述型字段
...
# 查找并替换函数
# 匹配从 "def get_chapter_content_template" 到下一个 "def " 之间的内容
pattern = r'def get_chapter_content_template\(section_title\):.*?(?=\ndef )'
...
if match:
    ...
    print('[OK] 补丁应用成功!')
    print(f'  替换了 get_chapter_content_template 函数')
else:
    print('[ERROR] 未找到目标函数')
